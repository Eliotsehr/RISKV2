...
public class Interface {
	...
	*/
	 ...
	 * 2 - Mode choix du territoire � attaquer
	 * 3 - Mode d�placement choix du territoire de d�part
	 * 4 - Mode d�placement choix du territoire d'arriv�
	 */
	private int mode = 0;
	...
	/**
	 *
	 * @param xMax int largeur de la fen�tre
	 * @param yMax int hauteur de la fen�tre
	...
	/**
	 * Interface qui permet de r�cup�rer les soldats, les cavaliers, les canons
	 */
	...
	/**
	 * Affiche des informations en haut de l'�cran
	...
	public void infosHaut(int message)
	{
		...
		{
		case 0://Selection
			StdDraw.text(0.5, 0.97, "Territoire s�l�ctionn�: "+territoire1.getNom());
			break;
		case 1://Attaque
		...
		case 3:
			...
			break;
		case 4:
			StdDraw.text(0.5, 0.97, "D�placement depuis "+territoire1.getNom()+" vers "+territoire2.getNom());
			break;
		case 5:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreTroupesDeploiement+" troupes � placer !");
			break;
		case 7:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreCanons+" canons � placer !");
			break;
		case 8:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreCavaliers+" cavaliers � placer !");
			break;
		case 9:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreSoldats+" soldats � placer !");
			break;
		case 10:
			StdDraw.text(0.5, 0.97, "Territoire s�l�ctionn�: "+territoire2.getNom());
			break;
		case 11:
			StdDraw.text(0.5, 0.97, "Tu as s�l�ction� un soldat !");
			break;
		case 12:
			StdDraw.text(0.5, 0.97, "Tu as s�l�ction� un cavalier !");
			break;
		case 13:
			StdDraw.text(0.5, 0.97, "Tu as s�l�ction� un canon !");
			break;
	...
	/**
	 * Affiche des informations en bas de l'�cran
	...
	public void infosBas(int message)
	{
		...
		{
		case 0:
			StdDraw.text(0.5, 0.03, "S�l�ctione un de tes territoires");
			break;
		case 1:
			StdDraw.text(0.5, 0.03, "S�l�ctione un territoire adverse");
			break;
		case 2:
		...
		case 7:
			...
			break;
		case 8:
			StdDraw.text(0.5, 0.03, "Tu n'as pas assez de troupes pour �a !");
			break;
		case 9:
			StdDraw.text(0.5, 0.03, "Selectionne tes troupes d'attaque d'abord !");
			break;
		case 10:
			StdDraw.text(0.5, 0.03, "Cette troupe a d�j� effectu� tout ses d�placements !");
			break;
		default:
			break;
		}
	...
	/**
	 * Affiche la couche des pions avec le nombre d'unit�es correspondant � chaque pion
	 */
	public void couchePions()
	{
		if(risk.map == 0)
		{
			...
			pion(0.65, 0.81, risk.listeTerritoires.get(17).getProprietaire().getNumeroDeJoueur());//Sib�rie
			...
			pion(0.14, 0.51, risk.listeTerritoires.get(33).getProprietaire().getNumeroDeJoueur());//Am�rique Centrale
			...
			pion(0.26, 0.35, risk.listeTerritoires.get(35).getProprietaire().getNumeroDeJoueur());//Br�sil
			StdDraw.text(0.26,0.35, ""+risk.listeTerritoires.get(35).getNombreTroupesTotal());

			pion(0.22, 0.31, risk.listeTerritoires.get(36).getProprietaire().getNumeroDeJoueur());//P�rou
			...
			pion(0.73, 0.29, risk.listeTerritoires.get(38).getProprietaire().getNumeroDeJoueur());//Indon�sie
			StdDraw.text(0.73,0.29, ""+risk.listeTerritoires.get(38).getNombreTroupesTotal());

			pion(0.81, 0.32, risk.listeTerritoires.get(39).getProprietaire().getNumeroDeJoueur());//Nouvelle Guin�e
	...
	/**
	 * Affiche les fen�tres de confirmation d'attaque
	 */
	...
	/**
	 * Affiche la fen�tre de depalcement
	 */
	 ...
	 * @return false si on a cliqu� sur un �l�ment pertinent de la fen�tre du menu true sinon
	 */
	 ...
	 * @param sourisX valeur en x de la souris
	 * @param sourisY valeur en y de la souris
	 * @return false si on a cliqu� sur un �l�ment pertinent de la fen�tre de l'interface true sinon
	 */
	...
	public boolean cliqueChoix(double sourisX, double sourisY)
	{
		...
		else if((sourisX > 0.44 && sourisX < 0.57) && (sourisY > 0.06 && sourisY < 0.11))//D�placer
		{
	 ...
	 * @param sourisX valeur en x de la souris
	 * @param sourisY valeur en y de la souris
	 * @param mode Mode de verification pour savoir ce qu'il faut v�rifier (ex: est-ce que le territoire appartient au joueur)
	 * @return false si on a cliqu� sur un �l�ment pertinent de la fen�tre de l'interface qui rempli les conditions du mode, true sinon
	 */
	public boolean cliqueTerritoires(double sourisX, double sourisY, Joueur joueur, int mode)
	{
		if(risk.map == 0)
		{
			...
			else if((sourisX > 0.63 && sourisX < 0.68) && (sourisY > 0.80 && sourisY < 0.82))//Sib�rie
			{
			...
			else if((sourisX > 0.11 && sourisX < 0.17) && (sourisY > 0.45 && sourisY < 0.57))//Am�rique Centrale
			{
			...
			else if((sourisX > 0.23 && sourisX < 0.29) && (sourisY > 0.34 && sourisY < 0.37))//Br�sil
			{
				return verifications(joueur,risk.listeTerritoires.get(35),mode);
			}
			else if((sourisX > 0.2 && sourisX < 0.23) && (sourisY > 0.30 && sourisY < 0.32))//P�rou
			{
			...
			else if((sourisX > 0.68 && sourisX < 0.77) && (sourisY > 0.26 && sourisY < 0.32))//Indon�sie
			{
				return verifications(joueur,risk.listeTerritoires.get(38),mode);
			}
			else if((sourisX > 0.78 && sourisX < 0.83) && (sourisY > 0.30 && sourisY < 0.35))//Nouvelle Guinn�e
			{
	 ...
	 * @param sourisX valeur en x de la souris
	 * @param sourisY valeur en y de la souris
	 * @return true si on a cliqu� sur un des deux boutons false sinon
	 */
	public boolean cliqueConfirmationAttaque(double sourisX, double sourisY)
	{
		...
		else if((sourisX > 0.24 && sourisX < 0.44) && (sourisY > 0.19 && sourisY < 0.25))//Confirmer
		{
			...
			StdDraw.pause(2000);//On attend un petit peu avant d'afficher le r�sultat du combat
	...
	//DIVERS
	/**
	 * M�thodes regroupant tous les types de verifications nescessaires au bon fonctionnement du jeu
	 * @param joueur Joueur le joueur sur lequel on fait la v�rification
	 * @param territoire Territoire le territoire sur lequel on fait la v�rification
	 * @param mode indice de la verification qu'on veut faire
	 * @return true ou false selon ce qu'on v�rifie, il n'y a pas de sortie "type"
	 */
	public boolean verifications(Joueur joueur, Territoire territoire, int mode)
	{
		...
		case 3://Selection du territoire duquel le joueur fait le d�placement
		...
		case 4://Selection du territoire sur lequel on fait le d�placement
