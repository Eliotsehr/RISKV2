...
public class Interface {
	...
	*/
	 ...
	 * 0 - Deploiement
	 * 1 - Mode choix du territoire pour attaquer
	 * 2 - Mode choix du territoire à attaquer
	 * 3 - Mode déplacement choix du territoire de départ
	 * 4 - Mode déplacement choix du territoire d'arrivé 
	 */
	private int mode = 0;
	...
	/**
	 * 
	 * @param xMax int largeur de la fenêtre
	 * @param yMax int hauteur de la fenêtre
	 * @param couche int couche sur laquelle on travail
	 */
	public Interface(int xMax,int yMax, int couche) {
		this.xMax = xMax;
	...
	/**
	 * Interface qui permet de récupérer les soldats, les cavaliers, les canons
	 */
	public void coucheDeploiement()
	{
		StdDraw.picture(0.5, 0.5, "img/cadreChoixTroupes.png");
	...
	/**
	 * Affiche des informations en haut de l'écran
	 * @param message int  Valeur pour savoir quel type de message afficher
	 */
	public void infosHaut(int message)
	{
		...
		{
		case 0://Selection
			StdDraw.text(0.5, 0.97, "Territoire séléctionné: "+territoire1.getNom());
			break;
		case 1://Attaque
			StdDraw.text(0.5, 0.97, territoire1.getNom()+" attaque "+territoire2.getNom()+" !");
			break;
		...
		case 3:
			...
			break;
		case 4:
			StdDraw.text(0.5, 0.97, "Déplacement depuis "+territoire1.getNom()+" vers "+territoire2.getNom());
			break;
		case 5:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreTroupesDeploiement+" troupes à placer !");
			break;
		case 7:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreCanons+" canons à placer !");
			break;
		case 8:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreCavaliers+" cavaliers à placer !");
			break;
		case 9:
			StdDraw.text(0.5, 0.97, "Il te reste "+risk.nombreSoldats+" soldats à placer !");
			break;
		case 10:
			StdDraw.text(0.5, 0.97, "Territoire séléctionné: "+territoire2.getNom());
			break;
		case 11:
			StdDraw.text(0.5, 0.97, "Tu as séléctioné un soldat !");
			break;
		case 12:
			StdDraw.text(0.5, 0.97, "Tu as séléctioné un cavalier !");
			break;
		case 13:
			StdDraw.text(0.5, 0.97, "Tu as séléctioné un canon !");
			break;
	...
	public void infosBas(int message)
	{
		...
		{
		case 0:
			StdDraw.text(0.5, 0.03, "Séléctione un de tes territoires");
			break;
		case 1:
			StdDraw.text(0.5, 0.03, "Séléctione un territoire adverse");
			break;
		case 2:
			StdDraw.text(0.5, 0.03, "CE TERRITOIRE NE T'APPARTIENT PAS OU NE PEUT PAS ATTAQUER !");
			break;
		...
		case 9:
			...
			break;
		case 10:
			StdDraw.text(0.5, 0.03, "Cette troupe a déjà effectué tout ses déplacements !");
			break;
		default:
			break;
		}
	...
	/**
	 * Affiche la couche des pions avec le nombre d'unitées correspondant à chaque pion
	 */
	public void couchePions()
	{
		if(risk.map == 0)
		{
			...
			pion(0.65, 0.81, risk.listeTerritoires.get(17).getProprietaire().getNumeroDeJoueur());//Sibérie
			StdDraw.text(0.65,0.81, ""+risk.listeTerritoires.get(17).getNombreTroupesTotal());
			...
			pion(0.14, 0.51, risk.listeTerritoires.get(33).getProprietaire().getNumeroDeJoueur());//Amérique Centrale
			StdDraw.text(0.14,0.51, ""+risk.listeTerritoires.get(33).getNombreTroupesTotal());
			
			pion(0.2, 0.44, risk.listeTerritoires.get(34).getProprietaire().getNumeroDeJoueur());//Venezuela
			StdDraw.text(0.2,0.44, ""+risk.listeTerritoires.get(34).getNombreTroupesTotal());
			
			pion(0.26, 0.35, risk.listeTerritoires.get(35).getProprietaire().getNumeroDeJoueur());//Brésil
			StdDraw.text(0.26,0.35, ""+risk.listeTerritoires.get(35).getNombreTroupesTotal());
			
			pion(0.22, 0.31, risk.listeTerritoires.get(36).getProprietaire().getNumeroDeJoueur());//Pérou
			StdDraw.text(0.22,0.31, ""+risk.listeTerritoires.get(36).getNombreTroupesTotal());
			
			pion(0.23, 0.22, risk.listeTerritoires.get(37).getProprietaire().getNumeroDeJoueur());//Argentine
			StdDraw.text(0.23,0.22, ""+risk.listeTerritoires.get(37).getNombreTroupesTotal());
			
			pion(0.73, 0.29, risk.listeTerritoires.get(38).getProprietaire().getNumeroDeJoueur());//Indonésie
			StdDraw.text(0.73,0.29, ""+risk.listeTerritoires.get(38).getNombreTroupesTotal());
			
			pion(0.81, 0.32, risk.listeTerritoires.get(39).getProprietaire().getNumeroDeJoueur());//Nouvelle Guinée
			StdDraw.text(0.81,0.32, ""+risk.listeTerritoires.get(39).getNombreTroupesTotal());
	...
	/**
	 * Affiche les fenêtres de confirmation d'attaque
	 */
	public void confirmationAttaque()
	{
		StdDraw.picture(0.24, 0.5, "img/troupesDefense.png");
	...
	/**
	 * Affiche la fenêtre de depalcement
	 */
	public void coucheDeplacement()
	{
		StdDraw.picture(0.5, 0.5, "img/troupesDeplacement.png");
	 ...
	 * @param sourisX valeur en x de la souris
	 * @param sourisY valeur en y de la souris
	 * @return false si on a cliqué sur un élément pertinent de la fenêtre du menu true sinon
	 */
	public boolean cliqueMenu(double sourisX, double sourisY)
	{
		if((sourisX > 0.18 && sourisX < 0.82) && (sourisY > 0.51 && sourisY < 0.59))//Jeu multi hurisk
	 ...
	 * @param sourisX valeur en x de la souris
	 * @param sourisY valeur en y de la souris
	 * @return false si on a cliqué sur un élément pertinent de la fenêtre de l'interface true sinon
	 */
	public boolean cliqueDeploiement(double sourisX,double sourisY)
	{
		this.sourisX = 0;
	...
	public boolean cliqueChoix(double sourisX, double sourisY)
	{
		...
		else if((sourisX > 0.44 && sourisX < 0.57) && (sourisY > 0.06 && sourisY < 0.11))//Déplacer
		{
			mode = 3;//Deplacement 1
			couche = 5;//Modes
	 ...
	 * @param sourisX valeur en x de la souris
	 * @param sourisY valeur en y de la souris
	 * @param mode Mode de verification pour savoir ce qu'il faut vérifier (ex: est-ce que le territoire appartient au joueur)
	 * @return false si on a cliqué sur un élément pertinent de la fenêtre de l'interface qui rempli les conditions du mode, true sinon
	 */
	public boolean cliqueTerritoires(double sourisX, double sourisY, Joueur joueur, int mode)
	{
		if(risk.map == 0)
		{
			...
			else if((sourisX > 0.63 && sourisX < 0.68) && (sourisY > 0.80 && sourisY < 0.82))//Sibérie
			{
				return verifications(joueur,risk.listeTerritoires.get(17),mode);
			}
			else if((sourisX > 0.69 && sourisX < 0.73) && (sourisY > 0.43 && sourisY < 0.45))//Siam
			...
			else if((sourisX > 0.11 && sourisX < 0.17) && (sourisY > 0.45 && sourisY < 0.57))//Amérique Centrale
			{
				return verifications(joueur,risk.listeTerritoires.get(33),mode);
			}
			else if((sourisX > 0.16 && sourisX < 0.23) && (sourisY > 0.42 && sourisY < 0.45))//Venezuela
			...
			else if((sourisX > 0.23 && sourisX < 0.29) && (sourisY > 0.34 && sourisY < 0.37))//Brésil
			{
				return verifications(joueur,risk.listeTerritoires.get(35),mode);
			}
			else if((sourisX > 0.2 && sourisX < 0.23) && (sourisY > 0.30 && sourisY < 0.32))//Pérou
			{
				return verifications(joueur,risk.listeTerritoires.get(36),mode);
			}
			else if((sourisX > 0.19 && sourisX < 0.25) && (sourisY > 0.21 && sourisY < 0.23))//Argentine
			...
			else if((sourisX > 0.68 && sourisX < 0.77) && (sourisY > 0.26 && sourisY < 0.32))//Indonésie
			{
				return verifications(joueur,risk.listeTerritoires.get(38),mode);
			}
			else if((sourisX > 0.78 && sourisX < 0.83) && (sourisY > 0.30 && sourisY < 0.35))//Nouvelle Guinnée
			{
				return verifications(joueur,risk.listeTerritoires.get(39),mode);
			}
			else if((sourisX > 0.73 && sourisX < 0.81) && (sourisY > 0.13 && sourisY < 0.18))//Australie de l'Ouest
	 ...
	 * @param sourisX valeur en x de la souris
	 * @param sourisY valeur en y de la souris
	 * @return true si on a cliqué sur un des deux boutons false sinon
	 */
	public boolean cliqueConfirmationAttaque(double sourisX, double sourisY)
	{
		if((sourisX > 0.03 && sourisX < 0.23) && (sourisY > 0.19 && sourisY < 0.25))//Anuler
		...
		else if((sourisX > 0.24 && sourisX < 0.44) && (sourisY > 0.19 && sourisY < 0.25))//Confirmer
		{
			...
			StdDraw.pause(2000);//On attend un petit peu avant d'afficher le résultat du combat
	...
	//DIVERS
	/**
	 * Méthodes regroupant tous les types de verifications nescessaires au bon fonctionnement du jeu
	 * @param joueur Joueur le joueur sur lequel on fait la vérification
	 * @param territoire Territoire le territoire sur lequel on fait la vérification
	 * @param mode indice de la verification qu'on veut faire
	 * @return true ou false selon ce qu'on vérifie, il n'y a pas de sortie "type"
	 */
	public boolean verifications(Joueur joueur, Territoire territoire, int mode)
	{		
		//Ces deux lignes sont pour corriger le bug du StdDraw.isMousePressed()
		...
		case 3://Selection du territoire duquel le joueur fait le déplacement
		...
		case 4://Selection du territoire sur lequel on fait le déplacement
...
>>>>>>> 8f18bc4ec5a17ab68789e6bbe874098e90d372a3
